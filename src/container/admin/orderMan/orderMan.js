...
class OrderMan extends React.Component {
  ...
  handleSearch = (e) => {
    ...
    this.props.form.validateFields((err, values) => {
      ...
      // this.getUserorder({id:`${values.id}`||undefined,state:values.state||undefined})
      if(!values.id){
        _this.setState({filterData:data})
      }
      _this.setState({filterData:data.filter(item=>{
        if(values.state==''||values.state==undefined||!values.state){
          return item.id==values.id
        }
        return item.id==values.id&&item.orderState==values.state
      })})
    });
  }
  ...
  render() {
    ...
    const { data, pageSize,filterData } = this.state;
